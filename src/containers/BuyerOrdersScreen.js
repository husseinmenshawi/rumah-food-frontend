...
function BuyerOrdersScreen({ navigation }) {
  ...
  const fetchOrders = () => {
    setLoading(true);
    fetch(`http://${config.ipAddress}:3000/api/v1.0/order/buyer/me`, {
      method: "get",
      headers: {
  ...
  const Item = ({ item }) => (
    ...
    >
      ...
      <Text style={styles.itemSubText}>
        Delivery Date: {moment(item.orderDateTime).format("YYYY-MM-DD")}
      </Text>
      <Text style={styles.itemSubText}>
        Delivery Day: {moment(item.orderDateTime).format("dddd")}
      </Text>
      <Text style={styles.itemSubText}>
